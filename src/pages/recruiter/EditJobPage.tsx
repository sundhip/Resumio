...
import type { JobFormData, WorkMode, JobEmploymentType, SalaryPeriod, JobPosting } from '../../types';
import {
  Briefcase,
  ArrowLeft,
  Building2,
...
export const EditJobPage: React.FC<EditJobPageProps> = ({ jobId, onNavigate }) => {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      if (!jobId) {
        showToast('error', 'Error', 'No Job ID provided');
        onNavigate('recruiter-jobs');
        return;
      }
      setIsLoading(true);
      try {
        const res = await api.getRecruiterJobById(jobId);
        if (res.success && res.job) {
          ...
          setFormData({
            ...
            qualification: j.qualification || "Bachelor's Degree",
            salary_disclosed: j.salary_disclosed,
  ...
  const handleUpdate = async (andPublish?: boolean) => {
    if (!validate()) {
    ...
    try {
      const res = await api.updateJob(jobId, formData);
      if (res.success) {
        if (andPublish && originalJob?.status !== 'Published') {
          await api.publishJob(jobId);
          showToast('success', 'Job Updated & Published!', 'The posting is now active and live.');
        } else {
          showToast('success', 'Job Updated Successfully', 'Changes have been saved.');
        }
        onNavigate('recruiter-jobs');
      }
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Update role specifications, requirements, and compensation parameters.
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-card bg-slate-50 dark:bg-surface-dark-card border border-slate-200/80 dark:border-surface-dark-border text-xs space-y-3">
            ...
            <p className="text-slate-500 dark:text-slate-400 leading-relaxed text-[11px]">
              Updating this job posting will instantly reflect across search and matching feeds.
            </p>
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-card bg-white dark:bg-surface-dark-card border border-slate-200/80 dark:border-surface-dark-border space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <Badge variant={originalJob?.status === 'Published' ? 'success' : 'default'}>
                {originalJob?.status || 'Draft'}
              </Badge>
            </div>
